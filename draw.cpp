...
void renderGame(int score, string guessedWord, int badGuessCount, string badGuess = "")
{
    clearScreen();
    cout << "Current guess = " << guessedWord << endl;
    cout << "You have made bad " << badGuessCount << " guess(es)"
         << (badGuess.empty() ? "" : ": ") << badGuess << endl
         << getHangPicture(badGuessCount) << endl << "SCORE: " << score << endl;
}
...
void printGameOverInfo(string guessedWord, string word, int badGuessCount, int score)
{
    renderGame(score, guessedWord, badGuessCount);
    if (guessedWord != word) {
        int n = 0;
        while (true) {
            clearScreen();
